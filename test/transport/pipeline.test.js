...
test('zenlog.transport with a pipeline', async ({ same, teardown }) => {
  ...
  same(result, {
    ...
    msg: 'hello',
    service: 'zenlog' // this property was added by the transform
  })
})

test('zenlog.transport with targets containing pipelines', async ({ same, teardown }) => {
...
test('zenlog.transport with targets containing pipelines with levels defined and dedupe', async ({ same, teardown }) => {
  const destinationA = file()
  const destinationB = file()
  const transport = zenlog.transport({
    targets: [
