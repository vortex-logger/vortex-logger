...
test('custom levels can be specified on child bindings', async ({ equal }) => {
  const stream = sink()
  const logger = zenlog(stream).child({
    childMsg: 'ok'
  }, {
    customLevels: {
      foo: 35
...
test('custom level on one instance does not affect other instances', async ({ equal }) => {
  ...
  equal(typeof zenlog().foo, 'undefined')
})

test('setting level below or at custom level will successfully log', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ customLevels: { foo: 35 } }, stream)
  instance.level = 'foo'
  instance.info('nope')
  instance.foo('bar')
...
test('custom level below level threshold will not log', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ customLevels: { foo: 15 } }, stream)
  instance.level = 'info'
  instance.info('bar')
  instance.foo('nope')
  const { msg } = await once(stream, 'data')
