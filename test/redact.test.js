...
test('redact option – throws if not array', async ({ throws }) => {
  throws(() => {
    zenlog({ redact: 'req.headers.cookie' })
  })
})
...
test('redact.paths option – throws if not array', async ({ throws }) => {
  throws(() => {
    zenlog({ redact: { paths: 'req.headers.cookie' } })
  })
})
...
test('redact option – top level key', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['key'] }, stream)
  instance.info({
    key: { redact: 'me' }
  })
...
test('redact option – top level key next level key', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['key', 'key.foo'] }, stream)
  instance.info({
    key: { redact: 'me' }
  })
  const { key } = await once(stream, 'data')
...
test('redact option – next level key then top level key', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['key.foo', 'key'] }, stream)
  instance.info({
    key: { redact: 'me' }
  })
  const { key } = await once(stream, 'data')
...
test('redact option – object', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['req.headers.cookie'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact.paths option – object', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: { paths: ['req.headers.cookie'] } }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact.censor option – sets the redact value', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: { paths: ['req.headers.cookie'], censor: 'test' } }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact.censor option – can be a function that accepts value and path arguments', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: { paths: ['topLevel'], censor: (value, path) => value + ' ' + path.join('.') } }, stream)
  instance.info({
    topLevel: 'test'
  })
...
test('redact.censor option – can be a function that accepts value and path arguments (nested path)', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: { paths: ['req.headers.cookie'], censor: (value, path) => value + ' ' + path.join('.') } }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact.remove option – removes both key and value', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: { paths: ['req.headers.cookie'], remove: true } }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact.remove – top level key - object value', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: { paths: ['key'], remove: true } }, stream)
  instance.info({
    key: { redact: 'me' }
  })
...
test('redact.remove – top level key in child logger', async ({ equal }) => {
  ...
  const instance = zenlog(opts, stream).child({ key: { redact: 'me' } })
  instance.info('test')
  const o = await once(stream, 'data')
  equal('key' in o, false)
})
...
test('redact – supports last position wildcard paths', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['req.headers.*'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact – supports first position wildcard paths', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['*.headers'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact – supports first position wildcards before other paths', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['*.headers.cookie', 'req.id'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact – supports first position wildcards after other paths', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['req.id', '*.headers.cookie'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact – supports first position wildcards after top level keys', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['key', '*.headers.cookie'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact – supports top level wildcard', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['*'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact – supports top level wildcard and leading wildcard', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['*', '*.req'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redact – supports intermediate wildcard paths', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['req.*.cookie'] }, stream)
  instance.info({
    req: {
      id: 7915,
      method: 'GET',
...
test('redacts numbers at the top level', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['id'] }, stream)
  const obj = {
    id: 7915
  }
  instance.info(obj)
...
test('redacts booleans at the top level', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['maybe'] }, stream)
  const obj = {
    maybe: true
  }
  instance.info(obj)
...
test('redacts strings at the top level', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['s'] }, stream)
  const obj = {
    s: 's'
  }
  instance.info(obj)
...
test('does not redact primitives if not objects', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['a.b'] }, stream)
  const obj = {
    a: 42
  }
  instance.info(obj)
...
test('redacts null at the top level', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['n'] }, stream)
  const obj = {
    n: null
  }
  instance.info(obj)
...
test('supports bracket notation', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['a["b.b"]'] }, stream)
  const obj = {
    a: { 'b.b': 'c' }
  }
  instance.info(obj)
...
test('supports bracket notation with further nesting', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['a["b.b"].c'] }, stream)
  const obj = {
    a: { 'b.b': { c: 'd' } }
  }
  instance.info(obj)
...
test('supports bracket notation with empty string as path segment', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['a[""].c'] }, stream)
  const obj = {
    a: { '': { c: 'd' } }
  }
  instance.info(obj)
...
test('supports leading bracket notation (single quote)', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['[\'a.a\'].b'] }, stream)
  const obj = {
    'a.a': { b: 'c' }
  }
  instance.info(obj)
...
test('supports leading bracket notation (single-segment path)', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['[`a.a`]'] }, stream)
  const obj = {
    'a.a': { b: 'c' }
  }
  instance.info(obj)
...
test('child bindings are redacted using wildcard path', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['*.headers.cookie'] }, stream)
  instance.child({
    req: {
      method: 'GET',
      url: '/',
...
test('child bindings are redacted using wildcard and plain path keys', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['req.method', '*.headers.cookie'] }, stream)
  instance.child({
    req: {
      method: 'GET',
      url: '/',
...
test('redacts boolean at the top level', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: ['msg'] }, stream)
  const obj = {
    s: 's'
  }
...
test('redact safe stringify', async ({ equal }) => {
  const stream = sink()
  const instance = zenlog({ redact: { paths: ['that.secret'] } }, stream)
